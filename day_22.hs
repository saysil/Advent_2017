...
makeBlackBG :: (Int, Int) -> Window -> IO ()
makeBlackBG (x, y) win = mvWAddStr win y x "#"

makeWhiteBG :: (Int, Int) -> Window -> IO ()
makeWhiteBG (x, y) win = return ()--mvWAddStr win y x "."

makeColor :: (Int, Int) -> Board -> Window -> (Int, Int) -> IO ()
makeColor (x1, y1) board win (x, y)
        | search == True = makeBlackBG (x-x1, abs (y-y1)) win
        | search == False = makeWhiteBG (x-x1, abs (y-y1)) win
        where search = (isBlack (x, y) board)


setBG :: (Window, BoardAttr) -> IO ()
setBG (win, (board, params)) = do
        (x, y, w, h) <- readMVar params
        mapM_ (makeColor (x, y) board win) ([ (a, b) | a<-[x..x+w], b<-[y-h..y]])

antloop :: MVar (Int, Int, Int, Int) -> (Ant, Board) -> (Ant, Board)
antloop params (ant, board) = do
